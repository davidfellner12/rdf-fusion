            ...
            fn bsbm_business_intelligence_10000(c: &mut Criterion, benchmarking_context: RdfFusionBenchContext) {
                let target_partitions = benchmarking_context.options().target_partitions.unwrap();

                execute_benchmark(
                    c,
                    benchmarking_context,
                    &|benchmark: BsbmBenchmark<BusinessIntelligenceUseCase>, benchmark_context: BenchmarkContext| {
                        let mut queries = BsbmBusinessIntelligenceQueryName::list_queries()
                            .into_iter()
                            .map(|query_name| {
                                (
                                    format!("BSBM BI 10000 (target_partitions={target_partitions}) - {query_name}"),
                        ...
                        queries.push((
                            format!("BSBM BI 10000 (target_partitions={target_partitions}) - Query 64"),
            ...
            fn execute_benchmark(
                ...
                let benchmark =
                    BsbmBenchmark::<BusinessIntelligenceUseCase>::try_new(NumProducts::N10_000, None).unwrap();
                ...
                write_csv(&rows, "bsbm_bi_results.csv");
            }
